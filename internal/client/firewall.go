...
// EnrollRequest is the body sent to the agent enroll endpoint.
type EnrollRequest struct {
	DeviceID string            `json:"device_id"`
	Metadata map[string]string `json:"metadata,omitempty"`
}

// EnrollResponse is the response from the agent enroll endpoint.
...
func (c *Client) Enroll(deviceID, bootstrapToken string, metadata map[string]string) (*EnrollResponse, error) {
	body, err := json.Marshal(EnrollRequest{DeviceID: deviceID, Metadata: metadata})
	if err != nil {
